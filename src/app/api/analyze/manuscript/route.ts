...
export async function POST(req: NextRequest) {
  try {
    ...
    const credit = await checkCreditLimit(userId, "manuscript_citations");
    if (!credit.allowed) {
      return NextResponse.json(
    ...
    const { text: analysis, usage } = await generateManuscriptCitations(phiResult.censoredText, style);
    recordUsage(userId, "manuscript_citations", usage);
