...
export async function POST(req: NextRequest) {
  try {
    ...
    const credit = await checkCreditLimit(userId, "manuscript_writer");
    if (!credit.allowed) {
      return NextResponse.json(
    ...
    const { text: analysis, usage } = await generateManuscript(phiResult.censoredText, {
      format,
      citationsEnabled,
      citationStyle,
      brevity,
    ...
    recordUsage(userId, "manuscript_writer", usage);
